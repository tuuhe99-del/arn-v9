...
from arn.core.conflicts import extract_claim, find_conflicts
...
def check_embeddings_available() -> bool:
    """Check if semantic tests should run.

    CI and offline machines should not download Hugging Face models during a
    normal test run. Set ARN_RUN_SEMANTIC_TESTS=1 when the model is already
    available or downloads are allowed.
    """
    if os.environ.get("ARN_RUN_SEMANTIC_TESTS") != "1":
        return False
    try:
...
def test_claim_conflict_helpers():
    """Test lightweight claim extraction and contradiction metadata."""
    print("\n[TIER 1] CLAIM / CONTRADICTION HELPERS")
    print("-" * 40)
    claim = extract_claim("Mohamed prefers Python")
    if claim and claim.subject == "mohamed" and claim.relation == "preference" and claim.object == "python":
        results.ok("Claim extraction captures preference")
    else:
        results.fail("Claim extraction", f"Got: {claim}")
    conflicts = find_conflicts("Mohamed prefers JavaScript", [{"id": 1, "content": "Mohamed prefers Python", "context": {"time_context": "current"}, "created_at": 1.0}])
    if conflicts and conflicts[0]["old_claim"]["object"] == "python":
        results.ok("Conflict helper keeps old and new claims")
    else:
        results.fail("Conflict helper", f"Got: {conflicts}")
...
@requires_embeddings
def test_meaning_recall_preference():
    """The core promise: meaning-based recall, not keyword matching."""
    print("\n[TIER 2] MEANING-BASED RECALL PROMISE")
    print("-" * 40)
    tmp_dir = make_temp_dir()
    try:
        arn = ARNv9(data_dir=tmp_dir, use_embeddings=True, consolidation_threshold=999)
        arn.perceive("Mohamed prefers Python", importance=0.95, context={"time_context": "current", "tags": ["preference", "coding"]})
        recalled = arn.recall("what does the user like to code in?", top_k=3)
        if recalled and "python" in recalled[0]["content"].lower():
            results.ok("Semantic recall maps 'prefers Python' to 'like to code in'")
        else:
            results.fail("Semantic recall promise", f"Got: {recalled[:3] if recalled else recalled}")
        arn.close()
    finally:
        shutil.rmtree(tmp_dir, ignore_errors=True)
...
def main():
    print("=" * 60)
    print("ARN COMPREHENSIVE TEST SUITE")
    print("=" * 60)
    ...
    test_functions = [
        ...
        test_persistence,
        test_working_memory,
        test_claim_conflict_helpers,
        # Tier 2: require embeddings
        test_embedding_quality,
        test_prediction_error,
        test_consolidation,
        test_contradiction_detection,
        test_meaning_recall_preference,
