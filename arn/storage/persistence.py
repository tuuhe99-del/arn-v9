...
class StorageEngine:
    """
    Persistent storage backend for ARN.
    ...
    def __init__(self, data_dir: str, max_episodes: int = 4096,
        ...
        # Thread safety — must be set before _init_db which calls _get_conn
    ...
    def _init_db(self):
        """Create tables if they don't exist, migrating older schemas."""
        conn = self._get_conn()

        # Create base tables (no new columns yet — safe for old DBs)
        conn.executescript("""
        ...
        # Migrate: add new columns if absent (safe on both fresh and old DBs)
        existing_cols = {r[1] for r in conn.execute("PRAGMA table_info(episodes)")}
        for col, defn in [
            ("superseded_by", "INTEGER DEFAULT NULL"),
            ("valid_from",    "REAL DEFAULT NULL"),
            ("fact_key",      "TEXT DEFAULT NULL"),
        ]:
            if col not in existing_cols:
                conn.execute(f"ALTER TABLE episodes ADD COLUMN {col} {defn}")

        # Add index on fact_key after ensuring column exists
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_episodes_fact_key ON episodes(fact_key)"
        )
    ...
    def supersede_episode(self, old_id: int, new_id: int, fact_key: str):
        """
        Mark old_id as superseded by new_id for a given fact_key.
        Both rows are kept — old one gets superseded_by set so recall
        can penalise it correctly.
        """
        conn = self._get_conn()
        now = time.time()
        conn.execute(
            "UPDATE episodes SET superseded_by=?, fact_key=? WHERE id=?",
            (new_id, fact_key, old_id)
        )
        conn.execute(
            "UPDATE episodes SET fact_key=?, valid_from=? WHERE id=?",
            (fact_key, now, new_id)
        )
        conn.commit()

    def get_episodes_by_fact_key(self, fact_key: str) -> List[dict]:
        """Return all episodes sharing a fact_key, newest first."""
        conn = self._get_conn()
        rows = conn.execute(
            "SELECT * FROM episodes WHERE fact_key=? ORDER BY created_at DESC",
            (fact_key,)
        ).fetchall()
        return [self._row_to_episode(r) for r in rows]
