...
VALID_TIERS = {"nano", "small", "balanced", "base", "base-e5", "large"}


def _parse_tier(argv=None):
    argv = list(sys.argv[1:] if argv is None else argv)
    for arg in argv:
        if arg in VALID_TIERS:
            return arg
    return os.environ.get("ARN_EMBEDDING_TIER", "nano")
...
def main():
    print("=" * 70)
    print(f"ARN ADVERSARIAL STRESS TEST — TIER: {TIER}")
    print("=" * 70)
